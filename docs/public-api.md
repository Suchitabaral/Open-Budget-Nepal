...
For exact parameters and response fields, use the generated [OpenAPI document](/api/openapi.json) or [interactive documentation](/api/docs).

## Examples

```bash
curl "http://localhost:3000/api/v1/projects?fiscalYear=2081%2F82&page=1&limit=10&sort=name_asc"
curl "http://localhost:3000/api/v1/contractors?q=construction&limit=25"
curl "http://localhost:3000/api/v1/contracts?procurementCategory=Works&sort=amount_desc"
curl "http://localhost:3000/api/v1/watchdog/findings?severity=High&limit=25"
```
