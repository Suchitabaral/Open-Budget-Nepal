...
test('OpenAPI document exposes only versioned implemented paths', () => {
  ...
  assert.ok(openApiDocument.paths['/budgets/overview']);
  assert.ok(openApiDocument.paths['/budgets/local-levels/{localLevelId}']);
  assert.equal(Object.keys(openApiDocument.paths).length, 26);
});
